...
// Run executes an agent with retries and timeouts, forwarding to RunWithFeedbackLoop with a nil verifier.
func (c *Client) Run(ctx context.Context, content *genai.Content) (string, error) {
	return c.RunWithFeedbackLoop(ctx, content, nil)
}
